...
var Basestar = module.exports = function () {
  var basestar = {};

  basestar.__proto__ = EventEmitter.prototype;                                                                                                                           
  ...
  basestar.proxyMethods = function(methods, target, source, force) {
    if (force == null) {
      force = false;
    }
  ...
  //
  // Returns the source
  basestar.defineEvent = function(opts) {
  ...
  basestar.defineAdaptorEvent = function(opts) {
    return basestar._proxyEvents(opts, this.connector, this.connection);
  };
  ...
  basestar.defineDriverEvent = function(opts) {
    return basestar._proxyEvents(opts, opts.connection, opts.device);
  };

  basestar._proxyEvents = function(opts, source, target) {
    opts = (typeof opts === 'string') ? { eventName: opts } : opts;

    opts.source = source;
    opts.target = target

    return basestar.defineEvent(opts);
  }
